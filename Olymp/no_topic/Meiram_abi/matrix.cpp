...
Matrix* rand(const int numRows, const int numCols)
{
    ...
    cur -> mat = new float[size];
    cur -> numRows = numRows, cur -> numCols = numCols;
    for (int i = 0; i < size; ++i) {
        cur -> mat[i] = 1 + (rand () % 100);
    }
    return cur;
}
...
Matrix* add(const Matrix* A, const Matrix* B)
{
    ...
    cur -> mat = new float[size];
    cur -> numCols = A -> numCols, cur -> numRows = A -> numRows;
    for (int i = 0; i < size; ++i) {
...
Matrix* multiply(const Matrix* A, const Matrix* B)
{
    ...
    Matrix* cur = new Matrix;
    cur -> numCols = B -> numCols, cur -> numRows = A -> numRows;
...
Matrix* multiply(const Matrix* A, const Matrix* B, const Matrix* C)
{
    ...
    int T1 = getTime(A -> numRows, A -> numCols, B -> numRows, B -> numCols) + getTime(A -> numRows, B -> numCols, C -> numRows, C -> numCols),
        T2 = getTime(B -> numRows, B -> numCols, C -> numRows, C -> numCols) + getTime(A -> numRows, A -> numCols, B -> numRows, C -> numCols);
...
Matrix* multiply(const Matrix* A, const float c)
{
    if (A == NULL)
        return NULL;
    Matrix* cur = new Matrix;
    cur -> numRows = A -> numRows, cur -> numCols = A -> numCols;
    int size = (A -> numRows) * (A -> numCols);
    cur -> mat = new float[size];
    for (int i = 0; i < size; ++i) {
        cur -> mat[i] = A -> mat[i] * c;
...
Matrix* load(const char* fname)
{
    ...
    int size = (cur -> numRows) * (cur -> numCols);
    cur -> mat = new float[size];
    for (int i = 0; i < size; ++i) {
        InFile >> cur -> mat[i];
    }
...
SparseMatrix* transpose(const SparseMatrix* A)
{
    ...
    cur -> numCols = A -> numCols, cur -> numRows = A -> numRows;
    cur -> mat = new list <SparseElementTriplet>;
    for (list <SparseElementTriplet> :: iterator it = A -> mat -> begin(); it != A -> mat -> end(); ++it) {
...
SparseMatrix* spmultiply(const SparseMatrix* A, const SparseMatrix* B, const SparseMatrix* C)
{
    ...
    int T1 = getTime(A -> numRows, A -> numCols, B -> numRows, B -> numCols) + getTime(A -> numRows, B -> numCols, C -> numRows, C -> numCols);
    int T2 = getTime(B -> numRows, B -> numCols, C -> numRows, C -> numCols) + getTime(A -> numRows, A -> numCols, B -> numRows, C -> numCols);
