...
struct InStream
{
  ...
  void readEof();
  void readEoln();
  void readEoln( int i, int n ); // rea/dSpace or readEoln
  int readSpace();
  bool seekEoln();
  bool seekEof();
  void skipBlanks();
...
long long InStream::readLong() {
  return readLong(LLONG_MIN, LLONG_MAX);
}
ullong InStream::readULong() {
  return readULong(0ULL, ULLONG_MAX);
}
