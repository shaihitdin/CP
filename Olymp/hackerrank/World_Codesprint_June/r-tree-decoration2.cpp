...
inline void dfs (int v) {
	...
	for (int i = 1; i <= ft; ++i) {
		for (int j = 1; j <= cc[i]; ++j) {
			calc2[i][j] = 1;
			for (auto to : g[v])
				calc2[i][j] = ll (calc2[i][j]) * dp[to][i][j] % bpr;
		}
	}
	for (int pt = 1 - (v == 1); pt <= ft; ++pt) {
		for (int c = 1 - (v == 1); c <= 20; ++c) {
			int kh = k - c;
			for (int i = 1; i <= ft; ++i) {
				if (i == pt)	
					continue;
				for (int j = 1; j <= cc[i] && j <= kh; ++j) {
					int rval = gcnk[kh][j];
					rval = ll(rval) * tt[cc[i]][j] % bpr;
					rval = ll(rval) * calc2[i][j] % bpr;
					/*
					for (auto to : g[v])
						rval = ll(rval) * dp[to][i][j] % bpr;
					*/
					add (dp[v][pt][c], rval);
				}
			}
...
int main() {
	...
	for (int i = 0; i < K; ++i) {
		...
		for (int j = 1; j <= 20 && j <= i; ++j)
			add (gcnk[i][j] = gcnk[i - 1][j], gcnk[i - 1][j - 1]);
