...
/*
inline node * Build (const int &Tx1, const int &Ty1, 
...
node * Build (const int &Tx1, const int &Ty1, 
				const int &Tx2, const int &Ty2, const int &v) {
	node *n_n;
	n_n = &init[v];	
	mx = max (v, mx);
	if (Tx1 == Tx2 && Ty1 == Ty2) {
...
ll get (node *t, const int &Tx1, const int &Ty1, 
				const int &Tx2, const int &Ty2, 
...
void upd (node *t, const int &Tx1, const int &Ty1, 
				const int &Tx2, const int &Ty2, 
				const int &x1, const int &y1, 
				const int &x2, const int &y2, const int &penalty) {
	push (t, (Tx2 - Tx1 + 1) * (Ty2 - Ty1 + 1));
	if (Tx1 > x2 || Tx2 < x1 || Ty1 > y2 || Ty2 < y1)
		return;
	if (x1 <= Tx1 && y1 <= Ty1 && Tx2 <= x2 && Ty2 <= y2) {
