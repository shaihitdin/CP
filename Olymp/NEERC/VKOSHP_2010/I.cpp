...
inline int get (const node *t, const int &Tl, const int &Tr, const int &l, const int &r) {
	if (Tr < l || Tl > r)
		return 0;
	if (l <= Tl && Tr <= r)
		return t -> sum;
...
inline node * upd (const node *o_root, const int &l, const int &r, const int &x, const int &to) {
	if (l == r)
		return new node (to);
	int mid = MID (l, r);
	if (l <= x && x <= mid)
...
inline int Get (node *t, const int &l, const int &k, const int &Tl, const int &Tr) {
	if (Tl == Tr)
		return (k == t -> sum) ? Tl : 0;
	int mid = MID (Tl, Tr);
	if (k <= t -> l -> sum)
		return Get (t -> l, l, k, Tl, mid);
	else
		return Get (t -> r, l, k - t -> l -> sum, mid + 1, Tr);
}
...
int main () {
	...
	for ( int i = 1; i <= q; ++i) {
		...
		l = ((l + p) % n) + 1;
		k = ((k + p) % m) + 1;
		int ans = Get (roots[l], l, k, 0, sz1 - 1);
		printf ("%d\n", ans);
		p = ans;
		/*
