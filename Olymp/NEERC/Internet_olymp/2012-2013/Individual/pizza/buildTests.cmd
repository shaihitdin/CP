...
if EXIST %problem%_%author%.dpr (dcc32 -cc %problem%_%author%.dpr || goto CompSourceError)
...
for %%i in (tests\??) do (
    ...
    if EXIST %problem%_%author%.exe (call %problem%_%author%.exe || goto SourceRunningError)
    if EXIST %problem%_%author%.class (call java -Xmx512M -Xss64M -ea %problem%_%author% || goto SourceRunningError)
    if EXIST %problem%_%author%.py (call python %problem%_%author%.py || goto SourceRunningError)
