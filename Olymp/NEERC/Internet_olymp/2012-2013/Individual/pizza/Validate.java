...
public class Validate {
	...
	public void run() {
		...
		for (int i = 0; i < n; i++) {
			...
			int x = inf.nextInt();
			sumLen += s.length();
			ensureLimits(sumLen, 1, MAXS, "SumLength incorrect");
			ensureLimits(s.length(), 1, MAXS, "");
			ensureLimits(x, 1, MAXC, Integer.toString(i+2) + " line has incorrect x");
			for (int j = 0; j < s.length(); j++) {
				ensure(s.charAt(j) >= 'a' && s.charAt(j) <= 'z', Integer.toString(i+2) + " line has incorrect s");
			}
			ensure(!set.contains(s), "Repeated string " + s + " Line #" + Integer.toString(i+2));
			set.add(s);
			inf.nextLine();
		}
		for (int i = 0; i < m; i++) {
			...
			int x = inf.nextInt();
			int y = inf.nextInt();
			ensure("!".equals(s) || "?".equals(s), "Bad line #" + Integer.toString(1+n+i+1));
			ensureLimits(x, 1, n, Integer.toString(1+n+i+1) + " line has incorrect x");
			if ("?".equals(s)) {
				ensureLimits(y, 1, MAXS, Integer.toString(1+n+i+1) + " line has incorrect y");
			} else {
				ensureLimits(y, -MAXC, MAXC, Integer.toString(1+n+i+1) + " line has incorrect y");
			}
			inf.nextLine();
		}
		inf.close();
